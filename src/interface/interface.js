...
export class Interface {
  constructor(sim, acoes) {
    ...
    document.getElementById('app').innerHTML = `
      <main class="jogo" aria-label="Mercadinho do Bairro">
        ...
        <footer class="rodape">
          <div class="inventario"><span class="cesta-icone">${icone('cesta')}</span><div class="inventario-conteudo"><div class="inventario-titulo"><b>Sua cesta</b><span id="capacidade">0 / 6</span></div><div id="itens"></div></div></div>
  ...
  atualizar() {
    ...
    if (chave !== this.ultimoInventario) {
      ...
      this.el('itens').innerHTML = Array.from({ length: Math.min(s.capacidade, 8) }, (_, i) => `<span class="item ${inv[i] || ''}" title="${inv[i] ? PRODUTOS[inv[i]].nome : 'Espaço livre'}">${inv[i] ? icone(inv[i]) : '<i></i>'}</span>`).join('') + (s.capacidade > 8 ? `<span class="item-extra">+${s.capacidade - 8}</span>` : '');
